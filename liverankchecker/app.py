...
from utils.utils import read_markdown_file, handle_authentication, logout, apply_claneo_branding

# === ACB_COMPAT_WRAPPER: make apply_claneo_branding accept optional args ===
try:
    # Wenn bereits aus utils.utils importiert wurde:
    _ACB_ORIG = apply_claneo_branding  # type: ignore[name-defined]
    def apply_claneo_branding(title=None, subtitle=None, **kwargs):  # noqa: F811
        try:
            # Versuche zuerst: alte No-Arg-Signatur
            return _ACB_ORIG()
        except TypeError:
            # Falls die Originalfunktion doch Args akzeptiert:
            try:
                return _ACB_ORIG(title, subtitle, **kwargs)
            except Exception:
                # Branding darf niemals die App crashen
                pass
        except Exception:
            pass
except Exception:
    # Fallback: minimale eigene Implementierung
    def apply_claneo_branding(title=None, subtitle=None, **kwargs):  # noqa: F811
        try:
            import streamlit as st
            try:
...
# handle_authentication() kann Bool oder Tuple zurückgeben
...
# --- injected fallback: apply_claneo_branding ---
def apply_claneo_branding(title=None, subtitle=None, **kwargs):
    """Apply basic Claneo branding; works with or without args.
    - title/subtitle are optional (safe if called with 0 args).
    """
    try:
        import streamlit as st
        # Optional: Logo falls verfügbar (keine harte Abhängigkeit)
        try:
            st.logo("https://www.claneo.com/wp-content/uploads/Element-4.svg")
        except Exception:
            pass
